...
| 12 | ~~Long-Running Tasks~~ | **Skipped** — investigated, the MCP Tasks extension is a dead protocol feature (Decisions Log #89). No scope was built. |
| 13 | Future Sources | LinkedIn, HN Who's Hiring, Wellfound, Greenhouse, Lever, career pages — one at a time, same adapter interface. |

**Current phase: 12 — Long-Running Tasks, skipped (see Decisions Log #89).** Update this line as we progress.
...
Decisions made during Phase 12 investigation, approved 2026-09-26.

89. **Phase 12 (Long-Running Tasks) is skipped entirely — investigated, confirmed as a dead protocol feature, not built.** Verified directly against the installed SDK (`@modelcontextprotocol/server` 2.1.0), the same rigor as every other phase: every schema behind the MCP Tasks extension (`TaskCreationParams`, `TaskStatus`, `Task`, `CreateTaskResult`, `ClientTasksCapability`, `ServerTasksCapability`) is marked `@deprecated 2025-11-25 wire vocabulary with no SDK runtime; kept importable for interoperability only` — no `tasks/get`/`tasks/list`/`tasks/cancel`/`tasks/result` handler exists anywhere in the server package, and the client package's own method-dispatch table maps those methods to `null`. The MCP spec's 2026-07-28 revision promoted Tasks to a new standalone extension with a *different* method set (`tasks/update` replacing `tasks/result`/`tasks/list`) — that shape doesn't exist in the installed SDK either. Confirmed live, not assumed: two Claude Code feature requests asking for client-side Tasks support were closed as duplicates of requests already closed **"not planned."** No working create/poll/cancel/result flow exists on either end of the protocol, and Anthropic has declined to build the client side — there's nothing to build against, and no announced timeline to revisit. If this changes in a future SDK/client release, worth reopening; not worth carrying a half-built integration against schemas the SDK author has already marked dead.
90. **Real, independent fix found during the Phase 12 investigation: `run_now` no longer blocks until the triggered task finishes.** `discovery_cycle` in particular can take well over a minute (multiple rate-limited external requests, one per newly-found job needing detail-page enrichment) — long enough to risk the MCP client's default 60-second request timeout (`DEFAULT_REQUEST_TIMEOUT_MSEC`, confirmed in the SDK) if a human ever triggered it mid-conversation, which was already possible before this phase. Fixed by making `run_now`'s handler fire the task without awaiting it, returning `{status: "started", tasks: [...]}` immediately — the exact same fire-and-forget pattern the scheduler's own clock-triggered runs already use (Decisions Log #78's `scheduleRecurring` never blocks on a task either). The real outcome still lands in `scheduler_runs` once the background run finishes, exactly like an automatic run — `get_automation_status` is the way to check it afterward, same as before. Live-verified: the tool call itself now returns in under 100ms regardless of task, confirmed by triggering a real `discovery_cycle` run against the live site (the call returned in 0.08s; the real run finished ~5s later and was correctly recorded). The CLI's `run-now` command is intentionally left synchronous/blocking — a terminal command isn't subject to a client-side protocol timeout, and printing the real result immediately is the more useful behavior there.
